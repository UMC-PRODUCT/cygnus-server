...
1. **실행 전에** 기록 문서를 만든다 — 가설부터 적는다. 하네스에 따라:
   - 리그 하네스: `loadtest/scripts/new-run.sh <이름>` → `runs/<날짜>-<이름>/summary.md` 스캐폴드. 환경(커밋·이미지·스펙)은 스크립트가 자동으로 채운다.
...
실행 방법은 `loadtest/RUNBOOK.md`(리그) 와 `loadtest/README.md`(notice 전/후 비교), 전략 배경은 `docs/adr/013` 참조.
