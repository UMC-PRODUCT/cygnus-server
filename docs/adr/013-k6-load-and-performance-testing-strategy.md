...
> **갱신 노트 (2026-07-22, load-test v1):** 실행 코드 위치는 `loadtest/k6/` 로 확정하고, 개별 시나리오를 직접 `k6 run` 하는 대신 **단일 `script.js` entrypoint 가 `__ENV.PROFILE × __ENV.SCENARIO` 로 실행 대상을 고르는** 구조로 구현했다. 이 ADR 뒤쪽 예시에 나오는 `k6 run loadtest/scenarios/...` 형태는 `run-umc-k6 <profile> <scenario> <rate> <duration>` 로 대체되었다. v1 은 부하 유형 smoke/load/stress/soak 를 구현하고 spike/breakpoint 는 후속으로 둔다. 상세: `docs/superpowers/plans/2026-07-22-load-test-v1-simplification.md`.
