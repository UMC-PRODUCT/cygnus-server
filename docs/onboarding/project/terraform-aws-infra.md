...
현재 저장소의 Terraform 코드는 `infra/terraform` 아래에서 단계적으로 구현한다. 이 문서는 이미 추가된 root/env/module 파일의 책임과, 이후 ALB/IAM/ASG/RDS/DNS 모듈까지 확장할 때 따라야 할 운영 기준을 함께 설명한다.
...
주의: 같은 VPC, ALB, Route53 record를 둘 이상의 Terraform state가 동시에 소유하면 안 된다. dev/prod root를 분리해 구현하는 경우 shared 리소스 소유자를 먼저 정하고, 다른 root는 remote state output 또는 data source로만 참조한다.
...
`plan`에서 관리 대상이 `api.university.neordinary.com`, `dev.api.university.neordinary.com` 범위를 벗어나면 중단한다. 의도하지 않은 `destroy` 또는 `replace`가 있으면 적용하지 않는다.
