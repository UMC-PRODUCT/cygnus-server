...
공유 선언은 한 파일만 소유한다. `SharedGraphQlArchitectureTest`가 owner 중복과 Project schema의 재선언을 검사한다.
...
Project의 `ProjectApplicationForm`은 `Form`을, `ApplicationFormSection`은 `FormSection`을 구현한다.
질문과 옵션은 concrete wrapper를 만들지 않고 `FormQuestion`, `FormOption`을 직접 재사용한다.
