...
class FcmNotificationOutboxFlowTest {
    ...
    void fcm_request_event_outbox_relay_flow() {
        ...
        FcmNotificationCommandService service = new FcmNotificationCommandService(
            new OutboxDomainEventPublisher(requestOutboxPort, requestOutboxPort, serializer, Tracer.NOOP)
        );
        ...
        FcmNotificationRequestedEventListener listener = new FcmNotificationRequestedEventListener(
            ...
            new OutboxDomainEventPublisher(batchOutboxPort, batchOutboxPort, serializer, Tracer.NOOP)
        );
    ...
    void fcm_batch_delivery_runs_without_relay_transaction() {
        ...
        FcmSendBatchRequestedEventListener listener = new FcmSendBatchRequestedEventListener(
            ...
            new OutboxDomainEventPublisher(retryOutboxPort, retryOutboxPort, serializer, Tracer.NOOP),
    ...
    private static class SingleEventOutboxPort implements LoadEventOutboxPort {
        ...
        public Optional<EventOutbox> findByEventId(UUID eventId) {
            return eventOutbox.getEventId().equals(eventId) ? Optional.of(eventOutbox) : Optional.empty();
        }

        @Override
        public List<EventOutbox> listPublishable(int limit, Instant now) {
            if (
                (eventOutbox.getStatus() == EventOutboxStatus.PENDING
                    || eventOutbox.getStatus() == EventOutboxStatus.PROCESSING)
                    && !eventOutbox.getNextAttemptAt().isAfter(now)
            ) {
    ...
    private static class RecordingSaveEventOutboxPort implements SaveEventOutboxPort, LoadEventOutboxPort {
        ...
        public boolean saveIfAbsent(EventOutbox eventOutbox) {
            if (findByEventId(eventOutbox.getEventId()).isPresent()) {
                return false;
            }
            saved.add(eventOutbox);
            return true;
        ...
        @Override
        public Optional<EventOutbox> findByEventId(UUID eventId) {
            return saved.stream().filter(outbox -> outbox.getEventId().equals(eventId)).findFirst();
        }
