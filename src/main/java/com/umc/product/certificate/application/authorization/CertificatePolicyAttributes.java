package com.umc.product.certificate.application.authorization;

import com.umc.product.authorization.domain.policy.PolicyAttributeKey;
import com.umc.product.authorization.domain.policy.PolicyValue;
import com.umc.product.authorization.domain.policy.PolicyValueType;

public final class CertificatePolicyAttributes {

    public static final PolicyAttributeKey<PolicyValue.BooleanValue> SUPER_ADMIN =
        new PolicyAttributeKey<>("relation.superAdmin", PolicyValueType.BOOLEAN);
    public static final PolicyAttributeKey<PolicyValue.BooleanValue> ACTIVE_CENTRAL_CORE_IN_TARGET_GISU =
        new PolicyAttributeKey<>(
            "relation.activeCentralCoreInTargetGisu",
            PolicyValueType.BOOLEAN);

    private CertificatePolicyAttributes() {
    }
}
